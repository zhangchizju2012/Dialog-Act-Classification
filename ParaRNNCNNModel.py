...
# -*- coding: utf-8 -*-
"""
Created on Fri Feb 10 18:50:51 2017
...
class CNNModel(LanguageModel):
  ...
  def add_model(self, inputs, is_training):
      ...
      num_filters = 128
      pooled_outputs = []
      inputs_changed = tf.expand_dims(inputs, -1)
      ...
      with tf.variable_scope('Combine'):
        self.h_drop = tf.concat(1, [self.h_drop,self.output])
        
      l2_loss = tf.constant(0.0)
      with tf.name_scope("output"):
            self.W = tf.Variable(tf.truncated_normal([num_filters_total+50, 8], stddev=0.1), name="W")
...
def test_CNNModel(startType='Restart'):
  ...
  if startType == 'Restart':
      with tf.Graph().as_default():
          ...
          '''
          model_path = "ParaRNNCNNmodel.ckpt"
          saver = tf.train.Saver(tf.all_variables(), max_to_keep=3)
          init = tf.initialize_all_variables()
          sess.run(init)    
