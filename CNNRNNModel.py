...
# -*- coding: utf-8 -*-
"""
Created on Thu Feb  9 21:44:41 2017
...
class CNNModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    self.pad = tf.placeholder(tf.float32, [None, 1, 50, 1], name='pad')
  ...
  def create_feed_dict(self, input_batch, label_batch, dropout_keep_prob, batch_real_len):
    feed_dict = {
        ...
        self.pad: np.zeros([len(input_batch), 1, 50, 1])}
    return feed_dict
  ...
  def add_model(self, inputs, is_training):
      ...
      pooled_concat = []
      reduced = np.int32(np.ceil((self.max_document_length) * 1.0 / 4))
      for i, filter_size in enumerate(filter_sizes):
          with tf.name_scope('conv-maxpool-%s' % filter_size):    
              ...
              emb_pad = tf.concat(1, [pad_prio, inputs, pad_post])
              ...
              pooled = tf.nn.max_pool(h, ksize=[1, 4, 1, 1], strides=[1, 4, 1, 1], padding='SAME', name='pool')
      ...
      lstm_cell = tf.nn.rnn_cell.GRUCell(num_units=50)
      ...
      outputs, state = tf.nn.rnn(lstm_cell, inputs, initial_state=self.initial_state, sequence_length=self.real_len)
